...
bot = Bot('7156760678:AAE4uYHSJMsDIjmXFg-ztm7bs7LHOUfGkqE')
...
def start_buttons() -> ReplyKeyboardMarkup:
    ...
    button_6 = KeyboardButton(text='Foydalanuvchilar')
...
async def send_application(message: types.Message, state: FSMContext):
    ...
    await state.clear()
    await bot.send_message(chat_id=385419373, text=text)
...
@form_router.message(F.text == 'Foydalanuvchilar')
async def get_user_list(message: types.Message,):
    users = get_data()
    text = ""
    for user in users:
        text += f"Ism: {user[0]} \n Telefon_raqam: {user[1]} \n\n"
